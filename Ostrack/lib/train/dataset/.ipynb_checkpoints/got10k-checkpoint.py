...
class Got10k(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None):
        ...
        # seq_id is the index of the folder inside the got10k root path  这部分在训练过程中没用，注释！！
        ...
        #     # seq_ids = pandas.read_csv(file_path, header=None, dtype=np.int64).squeeze("columns").values.tolist() # 原代码中的
        #     seq_ids = list(range(0, len(self.sequence_list))) # 由于不是全部文件，got10k_vot_train_split.txt中是随机的一千个文件,我的只有前1000个，会发生数组越界
        ...
        #     self.sequence_list = random.sample(self.sequence_list, int(len(self.sequence_list)*data_fraction)) # 随机选择占比为data_fraction个训练样例

        for seq_id in seq_ids:
            self._read_target_visible(self._get_sequence_path(seq_id))

        
        self.sequence_meta_info = self._load_meta_info() # 将这个设置为全none，因为应该没啥用，且UAV 数据中没有这些信息
        # print(self.sequence_meta_info)
        self.seq_per_class = self._build_seq_per_class()
        #print(self.seq_per_class)
        self.class_list = list(self.seq_per_class.keys())
       # print(self.class_list)
        self.class_list.sort()
        self.get_UAV_Train_list()


    def get_name(self): # 待定
        return 'got10k'

    def get_UAV_Train_list(self):# 多于了 与本来的函数功能重复
        path = os.getcwd() + '/data/UAV/train' # 当前UAVtrain数据所在目录 多于了
        with open(os.path.join(path, 'list.txt')) as f:
            dir_list = list(csv.reader(f))
        dir_list = [dir_name[0] for dir_name in dir_list]
        return dir_list
        # print("dir_list_UAV:",dir_list)
    ...
    def _read_bb_anno(self, seq_path):
        ...
        # print("!!!gt:",gt)
        label = self.get_IR_label(seq_path)
        groundtruth = DataFrame(label["gt_rect"]).values
        # print("groundtruth:", groundtruth)
        return torch.tensor(groundtruth) # 如果int后续不行的话，可以换成float
    
    def get_IR_label(self, seq_path):
        # print(seq_path)
        IR_label_file = os.path.join(seq_path, "IR_label.json")
        # print(IR_label_file)
    ...
    def _read_target_visible(self, seq_path):
        ...
        label = self.get_IR_label(seq_path)
        exist = label["exist"]
        target_visible = torch.ByteTensor(exist)
        visible_ratio = torch.ByteTensor(exist).float()
        # print("target_visible:", target_visible)
        # print("visible_ratio:", visible_ratio)
        return target_visible, visible_ratio

    def _get_sequence_path(self, seq_id):
        # print("seq_path:", os.path.join(self.root, self.sequence_list[seq_id]))
